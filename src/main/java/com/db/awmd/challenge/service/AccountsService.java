  ...
  public boolean transferAmount(String fromAccountId, String toAccountId, BigDecimal transferAmount ){
	  ...
	  synchronized(firstLock){
		  synchronized(secondLock){
			//Account should have sufficient balance and should not be negative after deduction of amount
			  if(fromAccount.hasSufficientBalance(transferAmount)){				  
				  ...
				  notifier.notifyAboutTransfer(fromAccount, transferAmount+" has been transferd to account "+toAccount.getAccountId());
				  notifier.notifyAboutTransfer(toAccount, transferAmount+" has been received from account "+fromAccount.getAccountId());				  
			  }else{
				  log.info("From account have insufficient balance. From account "+ fromAccountId+" should not end up with negtive balance : balance - "+fromAccount.getBalance()); 
				  throw new Exception("From account have insufficient balance. From account "+ fromAccountId+" should not end up with negtive balance");
			  }
		  }
	  }	  	 
	  return true;
